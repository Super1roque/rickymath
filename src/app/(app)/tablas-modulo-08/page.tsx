...
// ── Datos de la actividad (basados en la hoja "Tabla del 8", serie bonus
// "Tablas de Multiplicar"). La hoja trae la tabla completa (8×1 a 8×10) ya
...
const TRUCO = 'El 8 es doble, doble y doble otra vez. Para 8 × 3: 3 + 3 = 6, 6 + 6 = 12, 12 + 12 = 24.'

// El truco de arriba usa siempre el mismo ejemplo (8 × 3) — al explicar
...
function trucoPara(n: number): string {
  return `El 8 es doble, doble y doble otra vez. Para 8 × ${n}: ${n} + ${n} = ${n * 2}, ${n * 2} + ${n * 2} = ${n * 4}, ${n * 4} + ${n * 4} = ${n * 8}.`
}
...
const PRACTICA: ItemPractica[] = [
  ...
  { numero: 3, n: 9, resultado: BASE * 9 },
  { numero: 4, n: 3, resultado: BASE * 3 },
...
export default function TablasModulo08() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    guardarProgresoModulo(user.uid, perfilActivo.id, 'tablas-modulo-08', {
  ...
  return (
    <div className={fuenteJuego.className} style={{
      ...
      <div style={{
        position: 'relative',
        background: 'linear-gradient(135deg, #7c3aed, #2e1065)', padding: '1.75rem 1rem',
        textAlign: 'center', boxShadow: '0 5px 0 #1a0a3d', borderBottom: '4px solid #1a0a3d',
      }}>
        <BotonMenu href="/tablas-menu" />
        <div style={{ fontSize: '2.6rem', marginBottom: '0.3rem' }}>🕷️8️⃣</div>
        <h1 style={{
          fontSize: 'clamp(1.4rem, 5.5vw, 2.4rem)', fontWeight: 800, letterSpacing: '0.01em',
          textShadow: '2px 2px 0 #1a0a3d', margin: 0, color: 'white',
        }}>
          ¡Tabla del 8!
        </h1>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        <h2 style={{ fontSize: '1.3rem', fontWeight: 800, margin: '1.5rem 0 1rem', textAlign: 'center', color: '#2e1065' }}>
          ✏️ Ahora vos
        </h2>
        ...
        {!terminado && (
          <div
            aria-hidden="true"
            style={{ position: 'fixed', bottom: 10, right: 10, zIndex: 40, pointerEvents: 'none', filter: 'drop-shadow(0 3px 0 rgba(26,10,61,0.15))' }}
          >
          ...
          <div style={{
            ...
            <p style={{ opacity: 0.75, marginBottom: '1.5rem', fontSize: '1rem', fontWeight: 600 }}>
              {totalCorrectas === TOTAL_PREGUNTAS
                ? '¡Perfecto! Dominás la tabla del 8 🎮'
...
// Fila de bloques contables — un cuadradito por unidad, hasta 80, para que
// se vea como en la hoja (bloques de amatista con el "8" adentro) sin
  ...
  return (
    <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
      {Array.from({ length: cantidad }, (_, i) => (
        <div key={i} style={{
          width: 10, height: 10, borderRadius: 3, background: COLOR, color: 'white',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.4rem', fontWeight: 800,
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.6rem' }}>
        ...
        <p style={{ fontWeight: 800, fontSize: '1.05rem', margin: 0, flex: 1, color: '#2e1065' }}>La tabla del {BASE}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {TABLA.map(f => (
          <div key={f.n} style={{
            ...
            <span style={{ fontWeight: 800, fontSize: '0.95rem', color: '#2e1065', width: 78, flexShrink: 0 }}>
              {BASE} × {f.n} =
            </span>
...
function CajaTruco() {
  return (
    <div style={{
      background: '#f5f3ff', border: `3px dashed ${COLOR}`, borderRadius: 18, padding: '1rem',
      marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem',
    }}>
      <span style={{ fontSize: '1.6rem', flexShrink: 0 }}>💡</span>
      <div style={{ flex: 1 }}>
        <p style={{ margin: '0 0 0.2rem', fontWeight: 800, color: '#2e1065', fontSize: '0.95rem' }}>Truco</p>
        <p style={{ margin: 0, fontSize: '0.9rem', color: '#2e1065', fontWeight: 600 }}>{TRUCO}</p>
      </div>
...
function TarjetaPractica({ p, estado, onCambiar, onComprobar, onExplicarEstado }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '1.3rem', fontWeight: 800, margin: '0 0 0.5rem', color: '#2e1065' }}>
        {BASE} × {p.n} =
      </p>
      ...
      <input
        ...
        style={{
          ...
          background: '#f5f3ff', color: '#2e1065', fontSize: '1.15rem', fontWeight: 800, textAlign: 'center',
        }}
      />
