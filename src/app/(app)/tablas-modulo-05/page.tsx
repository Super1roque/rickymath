...
// ── Datos de la actividad (basados en la hoja "Tabla del 5", serie bonus
// "Tablas de Multiplicar"). La hoja trae la tabla completa (5×1 a 5×10) ya
...
const TRUCO = '¡La más fácil de ver! Si multiplicás por un número par termina en 0, y si es impar termina en 5.'
...
const PRACTICA: ItemPractica[] = [
  { numero: 1, n: 6, resultado: BASE * 6 },
  { numero: 2, n: 3, resultado: BASE * 3 },
  { numero: 3, n: 9, resultado: BASE * 9 },
  { numero: 4, n: 7, resultado: BASE * 7 },
  { numero: 5, n: 4, resultado: BASE * 4 },
]
...
export default function TablasModulo05() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    guardarProgresoModulo(user.uid, perfilActivo.id, 'tablas-modulo-05', {
  ...
  return (
    <div className={fuenteJuego.className} style={{
      minHeight: '100vh', overflowX: 'hidden', background: 'linear-gradient(180deg, #ea580c 0%, #fdba74 35%, #fff7ed 100%)',
      color: '#7c2d12', paddingBottom: '3rem',
    }}>
      <EstilosJuego />
      <div style={{
        position: 'relative',
        background: 'linear-gradient(135deg, #ea580c, #7c2d12)', padding: '1.75rem 1rem',
        textAlign: 'center', boxShadow: '0 5px 0 #431407', borderBottom: '4px solid #431407',
      }}>
        <BotonMenu href="/tablas-menu" />
        <div style={{ fontSize: '2.6rem', marginBottom: '0.3rem' }}>🟨5️⃣</div>
        <h1 style={{
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '1.5rem 1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 110 }}>
            <BarraProgreso completadas={totalEvaluadas} total={TOTAL_PREGUNTAS} color="#ea580c" />
          </div>
        </div>
        ...
        <h2 style={{ fontSize: '1.3rem', fontWeight: 800, margin: '1.5rem 0 1rem', textAlign: 'center', color: '#7c2d12' }}>
          ✏️ Ahora vos
        </h2>
          ...
          <div style={{
            ...
            <p style={{ opacity: 0.75, marginBottom: '1.5rem', fontSize: '1rem', fontWeight: 600 }}>
              {totalCorrectas === TOTAL_PREGUNTAS
                ? '¡Perfecto! Dominás la tabla del 5 🎮'
...
// Fila de bloques contables — un cuadradito por unidad, hasta 50, para que
// se vea como en la hoja (bloques de magma con el "5" adentro) sin
  ...
  return (
    <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
      {Array.from({ length: cantidad }, (_, i) => (
        <div key={i} style={{
          width: 13, height: 13, borderRadius: 3, background: COLOR, color: 'white',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.48rem', fontWeight: 800,
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.6rem' }}>
        ...
        <p style={{ fontWeight: 800, fontSize: '1.05rem', margin: 0, flex: 1, color: '#7c2d12' }}>La tabla del {BASE}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {TABLA.map(f => (
          <div key={f.n} style={{
            ...
            <span style={{ fontWeight: 800, fontSize: '0.95rem', color: '#7c2d12', width: 78, flexShrink: 0 }}>
              {BASE} × {f.n} =
            </span>
...
function CajaTruco() {
  return (
    <div style={{
      background: '#fff7ed', border: `3px dashed ${COLOR}`, borderRadius: 18, padding: '1rem',
      marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem',
    }}>
      <span style={{ fontSize: '1.6rem', flexShrink: 0 }}>💡</span>
      <div style={{ flex: 1 }}>
        <p style={{ margin: '0 0 0.2rem', fontWeight: 800, color: '#7c2d12', fontSize: '0.95rem' }}>Truco</p>
        <p style={{ margin: 0, fontSize: '0.9rem', color: '#7c2d12', fontWeight: 600 }}>{TRUCO}</p>
      </div>
...
function TarjetaPractica({ p, estado, onCambiar, onComprobar, onExplicarEstado }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '1.3rem', fontWeight: 800, margin: '0 0 0.5rem', color: '#7c2d12' }}>
        {BASE} × {p.n} =
      </p>
      ...
      <input
        ...
        style={{
          ...
          background: '#fff7ed', color: '#7c2d12', fontSize: '1.15rem', fontWeight: 800, textAlign: 'center',
        }}
      />
