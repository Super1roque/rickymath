...
// ── Datos de la actividad (basados en la hoja "Tabla del 2", serie bonus
// "Tablas de Multiplicar"). La hoja trae la tabla completa (2×1 a 2×10) ya
...
const TRUCO = 'Multiplicar por 2 es sumar el número consigo mismo. ¡Todos los resultados terminan en 0, 2, 4, 6 u 8!'
...
const PRACTICA: ItemPractica[] = [
  ...
  { numero: 2, n: 9, resultado: BASE * 9 },
  { numero: 3, n: 4, resultado: BASE * 4 },
  { numero: 4, n: 8, resultado: BASE * 8 },
  { numero: 5, n: 3, resultado: BASE * 3 },
]
...
export default function TablasModulo02() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    guardarProgresoModulo(user.uid, perfilActivo.id, 'tablas-modulo-02', {
  ...
  return (
    <div className={fuenteJuego.className} style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: '2.6rem', marginBottom: '0.3rem' }}>🐑2️⃣</div>
        <h1 style={{
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        <h2 style={{ fontSize: '1.3rem', fontWeight: 800, margin: '1.5rem 0 1rem', textAlign: 'center', color: '#1e3a8a' }}>
          ✏️ Ahora vos
        </h2>
          ...
          <div style={{
            ...
            <p style={{ opacity: 0.75, marginBottom: '1.5rem', fontSize: '1rem', fontWeight: 600 }}>
              {totalCorrectas === TOTAL_PREGUNTAS
                ? '¡Perfecto! Dominás la tabla del 2 🎮'
...
// Fila de bloques contables — un cuadradito por unidad, hasta 20, para que
// se vea como en la hoja (ovejas con el "2" adentro) sin depender de
// imágenes.
  ...
  return (
    <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
      {Array.from({ length: cantidad }, (_, i) => (
        <div key={i} style={{
          width: 18, height: 18, borderRadius: 4, background: COLOR, color: 'white',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.58rem', fontWeight: 800,
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.6rem' }}>
        ...
        <p style={{ fontWeight: 800, fontSize: '1.05rem', margin: 0, flex: 1, color: '#1e3a8a' }}>La tabla del {BASE}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {TABLA.map(f => (
          <div key={f.n} style={{
            ...
            <span style={{ fontWeight: 800, fontSize: '0.95rem', color: '#1e3a8a', width: 78, flexShrink: 0 }}>
              {BASE} × {f.n} =
            </span>
...
function CajaTruco() {
  return (
    <div style={{
      background: '#eff6ff', border: `3px dashed ${COLOR}`, borderRadius: 18, padding: '1rem',
      marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem',
    }}>
      <span style={{ fontSize: '1.6rem', flexShrink: 0 }}>💡</span>
      <div style={{ flex: 1 }}>
        <p style={{ margin: '0 0 0.2rem', fontWeight: 800, color: '#1e3a8a', fontSize: '0.95rem' }}>Truco</p>
        <p style={{ margin: 0, fontSize: '0.9rem', color: '#1e3a8a', fontWeight: 600 }}>{TRUCO}</p>
      </div>
...
function TarjetaPractica({ p, estado, onCambiar, onComprobar, onExplicarEstado }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '1.3rem', fontWeight: 800, margin: '0 0 0.5rem', color: '#1e3a8a' }}>
        {BASE} × {p.n} =
      </p>
      ...
      <input
        ...
        style={{
          ...
          background: '#eff6ff', color: '#1e3a8a', fontSize: '1.15rem', fontWeight: 800, textAlign: 'center',
        }}
      />
