...
export default function CuartoModulo07() {
  ...
  function comprobarItem(p: ItemArea, boton: HTMLButtonElement) {
    const actual = items[p.numero]
    if (actual.evaluado || actual.valor.trim() === '') return
    const correcto = Number(actual.valor.trim()) === p.area
    if (correcto) { reproducirCorrecto(); disparar(String(p.area), boton.getBoundingClientRect()) } else reproducirIncorrecto()
