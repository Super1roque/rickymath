...
// ── Datos de la actividad (basados en la hoja "Tabla del 10", serie bonus
// "Tablas de Multiplicar"). La hoja trae la tabla completa (10×1 a 10×10)
// ya resuelta como referencia visual — no es una clave a ocultar, es la
// lección en sí. Solo la sección "Ahora tú" (5 cuentas sueltas) es la
// parte que el chico completa y la app corrige. Es el último módulo de la
// serie bonus — la hoja original cierra con "¡Dominaste las tablas!".
...
const TRUCO = 'La más rápida de todas: escribí el número y agregale un cero. ¡Listo!'
...
const PRACTICA: ItemPractica[] = [
  ...
  { numero: 2, n: 7, resultado: BASE * 7 },
  { numero: 3, n: 2, resultado: BASE * 2 },
  { numero: 4, n: 9, resultado: BASE * 9 },
  { numero: 5, n: 6, resultado: BASE * 6 },
]
...
export default function TablasModulo10() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    guardarProgresoModulo(user.uid, perfilActivo.id, 'tablas-modulo-10', {
  ...
  return (
    <div className={fuenteJuego.className} style={{
      minHeight: '100vh', overflowX: 'hidden', background: 'linear-gradient(180deg, #ca8a04 0%, #fde68a 35%, #fefce8 100%)',
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        <h2 style={{ fontSize: '1.3rem', fontWeight: 800, margin: '1.5rem 0 1rem', textAlign: 'center', color: '#713f12' }}>
          ✏️ Ahora vos
        </h2>

        <div style={{
          display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.85rem', marginBottom: '1.5rem',
        }}>
          {PRACTICA.map(p => (
        ...
        {terminado && totalCorrectas === TOTAL_PREGUNTAS && (
          <div style={{
            textAlign: 'center', background: 'linear-gradient(135deg, #fefce8, #fef9c3)', border: '3px solid #eab308',
            borderRadius: 20, padding: '1.25rem', marginBottom: '1.5rem', boxShadow: '0 4px 0 rgba(180,83,9,0.2)',
          }}>
            <p style={{ fontSize: '1.9rem', margin: '0 0 0.3rem' }}>🏆</p>
            <p style={{ fontWeight: 800, fontSize: '1.15rem', color: '#854d0e', margin: 0 }}>¡Dominaste las tablas del 1 al 10!</p>
          </div>
        )}
          ...
          <div style={{
            ...
            <p style={{ opacity: 0.75, marginBottom: '1.5rem', fontSize: '1rem', fontWeight: 600 }}>
              {totalCorrectas === TOTAL_PREGUNTAS
                ? '🎉 ¡Completaste toda la serie bonus de Tablas de Multiplicar! 🏆'
...
// Fila de bloques contables — un cuadradito por unidad, hasta 100, para
// que se vea como en la hoja (bloques de oro con el "10" adentro) sin
  ...
  return (
    <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
      {Array.from({ length: cantidad }, (_, i) => (
        <div key={i} style={{
          width: 8, height: 8, borderRadius: 2, background: COLOR, color: 'white',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.3rem', fontWeight: 800,
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.6rem' }}>
        ...
        <p style={{ fontWeight: 800, fontSize: '1.05rem', margin: 0, flex: 1, color: '#713f12' }}>La tabla del {BASE}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {TABLA.map(f => (
          <div key={f.n} style={{
            ...
            <span style={{ fontWeight: 800, fontSize: '0.95rem', color: '#713f12', width: 84, flexShrink: 0 }}>
              {BASE} × {f.n} =
            </span>
            <div style={{ flex: 1, overflowX: 'auto' }}>
              <FilaBloques cantidad={f.resultado} />
            </div>
            <span style={{ fontWeight: 800, fontSize: '1.1rem', color: COLOR, width: 40, textAlign: 'right', flexShrink: 0 }}>
              {f.resultado}
            </span>
          </div>
        ))}
...
function CajaTruco() {
  return (
    <div style={{
      background: '#fefce8', border: `3px dashed ${COLOR}`, borderRadius: 18, padding: '1rem',
      marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem',
    }}>
      <span style={{ fontSize: '1.6rem', flexShrink: 0 }}>💡</span>
      <div style={{ flex: 1 }}>
        <p style={{ margin: '0 0 0.2rem', fontWeight: 800, color: '#713f12', fontSize: '0.95rem' }}>Truco</p>
        <p style={{ margin: 0, fontSize: '0.9rem', color: '#713f12', fontWeight: 600 }}>{TRUCO}</p>
      </div>
...
function TarjetaPractica({ p, estado, onCambiar, onComprobar, onExplicarEstado }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '1.3rem', fontWeight: 800, margin: '0 0 0.5rem', color: '#713f12' }}>
        {BASE} × {p.n} =
      </p>
      ...
      <input
        ...
        style={{
          ...
          background: '#fefce8', color: '#713f12', fontSize: '1.15rem', fontWeight: 800, textAlign: 'center',
        }}
      />
