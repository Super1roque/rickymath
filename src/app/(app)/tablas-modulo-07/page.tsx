...
// ── Datos de la actividad (basados en la hoja "Tabla del 7", serie bonus
// "Tablas de Multiplicar"). La hoja trae la tabla completa (7×1 a 7×10) ya
...
const TRUCO = '¡La tabla de la suerte! Memorizá los resultados clave como 7 × 7 = 49.'
...
export default function TablasModulo07() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    guardarProgresoModulo(user.uid, perfilActivo.id, 'tablas-modulo-07', {
  ...
  return (
    <div className={fuenteJuego.className} style={{
      minHeight: '100vh', overflowX: 'hidden', background: 'linear-gradient(180deg, #0891b2 0%, #67e8f9 35%, #ecfeff 100%)',
      color: '#164e63', paddingBottom: '3rem',
    }}>
      <EstilosJuego />
      <div style={{
        position: 'relative',
        background: 'linear-gradient(135deg, #0891b2, #164e63)', padding: '1.75rem 1rem',
        textAlign: 'center', boxShadow: '0 5px 0 #083344', borderBottom: '4px solid #083344',
      }}>
        <BotonMenu href="/tablas-menu" />
        <div style={{ fontSize: '2.6rem', marginBottom: '0.3rem' }}>💎7️⃣</div>
        <h1 style={{
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        <h2 style={{ fontSize: '1.3rem', fontWeight: 800, margin: '1.5rem 0 1rem', textAlign: 'center', color: '#164e63' }}>
          ✏️ Ahora vos
        </h2>
          ...
          <div style={{
            ...
            <p style={{ opacity: 0.75, marginBottom: '1.5rem', fontSize: '1rem', fontWeight: 600 }}>
              {totalCorrectas === TOTAL_PREGUNTAS
                ? '¡Perfecto! Dominás la tabla del 7 🎮'
...
// Fila de bloques contables — un cuadradito por unidad, hasta 70, para que
// se vea como en la hoja (bloques de diamante con el "7" adentro) sin
  ...
  return (
    <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
      {Array.from({ length: cantidad }, (_, i) => (
        <div key={i} style={{
          width: 11, height: 11, borderRadius: 3, background: COLOR, color: 'white',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.42rem', fontWeight: 800,
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.6rem' }}>
        ...
        <p style={{ fontWeight: 800, fontSize: '1.05rem', margin: 0, flex: 1, color: '#164e63' }}>La tabla del {BASE}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {TABLA.map(f => (
          <div key={f.n} style={{
            ...
            borderBottom: '1px dashed #a5f3fc',
          }}>
            <span style={{ fontWeight: 800, fontSize: '0.95rem', color: '#164e63', width: 78, flexShrink: 0 }}>
              {BASE} × {f.n} =
            </span>
...
function CajaTruco() {
  return (
    <div style={{
      background: '#ecfeff', border: `3px dashed ${COLOR}`, borderRadius: 18, padding: '1rem',
      marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem',
    }}>
      <span style={{ fontSize: '1.6rem', flexShrink: 0 }}>💡</span>
      <div style={{ flex: 1 }}>
        <p style={{ margin: '0 0 0.2rem', fontWeight: 800, color: '#164e63', fontSize: '0.95rem' }}>Truco</p>
        <p style={{ margin: 0, fontSize: '0.9rem', color: '#164e63', fontWeight: 600 }}>{TRUCO}</p>
      </div>
...
function TarjetaPractica({ p, estado, onCambiar, onComprobar, onExplicarEstado }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '1.3rem', fontWeight: 800, margin: '0 0 0.5rem', color: '#164e63' }}>
        {BASE} × {p.n} =
      </p>
      ...
      <input
        ...
        style={{
          ...
          background: '#ecfeff', color: '#164e63', fontSize: '1.15rem', fontWeight: 800, textAlign: 'center',
        }}
      />
