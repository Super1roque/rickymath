...
// ── Datos de la actividad (basados en la hoja "Tabla del 3", serie bonus
// "Tablas de Multiplicar"). La hoja trae la tabla completa (3×1 a 3×10) ya
...
const TRUCO = 'Suma el número tres veces: 3 × 4 es 4 + 4 + 4.'

// El truco de arriba usa siempre el mismo ejemplo (3 × 4) — al explicar
// OTRA pregunta (3 × 6, 3 × 2, etc.) eso confundía, porque parecía estar
// resolviendo una cuenta distinta a la que se acababa de responder. Esta
// versión arma el ejemplo con el número real de la pregunta.
function trucoPara(n: number): string {
  return `Suma el número tres veces: 3 × ${n} es ${n} + ${n} + ${n}.`
}
...
export default function TablasModulo03() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    guardarProgresoModulo(user.uid, perfilActivo.id, 'tablas-modulo-03', {
  ...
  return (
    <div className={fuenteJuego.className} style={{
      minHeight: '100vh', overflowX: 'hidden', background: 'linear-gradient(180deg, #d97706 0%, #fcd34d 35%, #fffbeb 100%)',
      ...
      <div style={{
        ...
        <div style={{ fontSize: '2.6rem', marginBottom: '0.3rem' }}>🐝3️⃣</div>
        <h1 style={{
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        <h2 style={{ fontSize: '1.3rem', fontWeight: 800, margin: '1.5rem 0 1rem', textAlign: 'center', color: '#78350f' }}>
          ✏️ Ahora vos
        </h2>
          ...
          <div style={{
            ...
            <p style={{ opacity: 0.75, marginBottom: '1.5rem', fontSize: '1rem', fontWeight: 600 }}>
              {totalCorrectas === TOTAL_PREGUNTAS
                ? '¡Perfecto! Dominás la tabla del 3 🎮'
...
// Fila de bloques contables — un cuadradito por unidad, hasta 30, para que
// se vea como en la hoja (florcitas con el "3" adentro) sin depender de
// imágenes.
  ...
  return (
    <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
      {Array.from({ length: cantidad }, (_, i) => (
        <div key={i} style={{
          width: 16, height: 16, borderRadius: 4, background: COLOR, color: 'white',
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.6rem' }}>
        ...
        <p style={{ fontWeight: 800, fontSize: '1.05rem', margin: 0, flex: 1, color: '#78350f' }}>La tabla del {BASE}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {TABLA.map(f => (
          <div key={f.n} style={{
            ...
            <span style={{ fontWeight: 800, fontSize: '0.95rem', color: '#78350f', width: 78, flexShrink: 0 }}>
              {BASE} × {f.n} =
            </span>
...
function CajaTruco() {
  return (
    <div style={{
      background: '#fffbeb', border: `3px dashed ${COLOR}`, borderRadius: 18, padding: '1rem',
      marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem',
    }}>
      <span style={{ fontSize: '1.6rem', flexShrink: 0 }}>💡</span>
      <div style={{ flex: 1 }}>
        <p style={{ margin: '0 0 0.2rem', fontWeight: 800, color: '#78350f', fontSize: '0.95rem' }}>Truco</p>
        <p style={{ margin: 0, fontSize: '0.9rem', color: '#78350f', fontWeight: 600 }}>{TRUCO}</p>
      </div>
...
function TarjetaPractica({ p, estado, onCambiar, onComprobar, onExplicarEstado }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '1.3rem', fontWeight: 800, margin: '0 0 0.5rem', color: '#78350f' }}>
        {BASE} × {p.n} =
      </p>
