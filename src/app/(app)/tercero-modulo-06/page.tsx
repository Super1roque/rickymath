...
export default function TerceroModulo06() {
  ...
  function comprobarFigura(p: PreguntaPerimetro, boton: HTMLButtonElement) {
    ...
    if (correcto) { reproducirCorrecto(); disparar(String(perimetro(p.lados)), boton.getBoundingClientRect()) } else reproducirIncorrecto()
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
          {FIGURAS.map((p, i) => (
            <TarjetaFigura key={p.numero} p={p} estado={figuras[p.numero]} color={COLORES[i % COLORES.length]}
              ...
              onComprobar={boton => comprobarFigura(p, boton)} onExplicarEstado={reaccionarRickyExplicar} />
          ))}
        </div>
