...
// ── Datos de la actividad (basados en la hoja "Tabla del 6", serie bonus
// "Tablas de Multiplicar"). La hoja trae la tabla completa (6×1 a 6×10) ya
...
const TRUCO = 'El 6 es el doble de la tabla del 3. Si sabés que 3 × 7 = 21, entonces 6 × 7 = 21 + 21 = 42.'

// El truco de arriba usa siempre el mismo ejemplo (3 × 7 → 6 × 7) — al
// explicar OTRA pregunta eso confundía, porque parecía estar resolviendo
// una cuenta distinta a la que se acababa de responder. Esta versión arma
// el ejemplo con el número real de la pregunta.
function trucoPara(n: number): string {
  return `El 6 es el doble de la tabla del 3. Si sabés que 3 × ${n} = ${3 * n}, entonces 6 × ${n} = ${3 * n} + ${3 * n} = ${6 * n}.`
}
...
const PRACTICA: ItemPractica[] = [
  { numero: 1, n: 4, resultado: BASE * 4 },
  { numero: 2, n: 8, resultado: BASE * 8 },
  { numero: 3, n: 3, resultado: BASE * 3 },
  { numero: 4, n: 9, resultado: BASE * 9 },
...
export default function TablasModulo06() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    guardarProgresoModulo(user.uid, perfilActivo.id, 'tablas-modulo-06', {
  ...
  return (
    <div className={fuenteJuego.className} style={{
      minHeight: '100vh', overflowX: 'hidden', background: 'linear-gradient(180deg, #15803d 0%, #86efac 35%, #f0fdf4 100%)',
      color: '#052e16', paddingBottom: '3rem',
    }}>
      <EstilosJuego />
      <div style={{
        position: 'relative',
        background: 'linear-gradient(135deg, #15803d, #052e16)', padding: '1.75rem 1rem',
        textAlign: 'center', boxShadow: '0 5px 0 #021b0d', borderBottom: '4px solid #021b0d',
      }}>
        <BotonMenu href="/tablas-menu" />
        <div style={{ fontSize: '2.6rem', marginBottom: '0.3rem' }}>💚6️⃣</div>
        <h1 style={{
          fontSize: 'clamp(1.4rem, 5.5vw, 2.4rem)', fontWeight: 800, letterSpacing: '0.01em',
          textShadow: '2px 2px 0 #021b0d', margin: 0, color: 'white',
        }}>
          ¡Tabla del 6!
        </h1>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '1.5rem 1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 110 }}>
            <BarraProgreso completadas={totalEvaluadas} total={TOTAL_PREGUNTAS} color="#15803d" />
          </div>
        </div>
        ...
        <h2 style={{ fontSize: '1.3rem', fontWeight: 800, margin: '1.5rem 0 1rem', textAlign: 'center', color: '#052e16' }}>
          ✏️ Ahora vos
        </h2>
        ...
        {!terminado && (
          <div
            aria-hidden="true"
            style={{ position: 'fixed', bottom: 10, right: 10, zIndex: 40, pointerEvents: 'none', filter: 'drop-shadow(0 3px 0 rgba(2,27,13,0.15))' }}
          >
        ...
        {terminado && (
          <div style={{
            ...
            <p style={{ fontSize: '1.7rem', fontWeight: 800, marginBottom: '0.3rem', color: '#052e16' }}>
              {totalCorrectas} de {TOTAL_PREGUNTAS} correctas
            </p>
            <p style={{ fontSize: '1.2rem', fontWeight: 700, marginBottom: '0.5rem', color: '#15803d' }}>
            ...
            <p style={{ opacity: 0.75, marginBottom: '1.5rem', fontSize: '1rem', fontWeight: 600 }}>
              {totalCorrectas === TOTAL_PREGUNTAS
                ? '¡Perfecto! Dominás la tabla del 6 🎮'
...
// Fila de bloques contables — un cuadradito por unidad, hasta 60, para que
// se vea como en la hoja (bloques de esmeralda con el "6" adentro) sin
  ...
  return (
    <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
      {Array.from({ length: cantidad }, (_, i) => (
        <div key={i} style={{
          width: 12, height: 12, borderRadius: 3, background: COLOR, color: 'white',
          display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.45rem', fontWeight: 800,
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.6rem' }}>
        ...
        <p style={{ fontWeight: 800, fontSize: '1.05rem', margin: 0, flex: 1, color: '#052e16' }}>La tabla del {BASE}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {TABLA.map(f => (
          <div key={f.n} style={{
            ...
            <span style={{ fontWeight: 800, fontSize: '0.95rem', color: '#052e16', width: 78, flexShrink: 0 }}>
              {BASE} × {f.n} =
            </span>
...
function CajaTruco() {
  return (
    <div style={{
      background: '#f0fdf4', border: `3px dashed ${COLOR}`, borderRadius: 18, padding: '1rem',
      marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem',
    }}>
      <span style={{ fontSize: '1.6rem', flexShrink: 0 }}>💡</span>
      <div style={{ flex: 1 }}>
        <p style={{ margin: '0 0 0.2rem', fontWeight: 800, color: '#052e16', fontSize: '0.95rem' }}>Truco</p>
        <p style={{ margin: 0, fontSize: '0.9rem', color: '#052e16', fontWeight: 600 }}>{TRUCO}</p>
      </div>
...
function TarjetaPractica({ p, estado, onCambiar, onComprobar, onExplicarEstado }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '1.3rem', fontWeight: 800, margin: '0 0 0.5rem', color: '#052e16' }}>
        {BASE} × {p.n} =
      </p>
      ...
      <input
        ...
        style={{
          ...
          background: '#f0fdf4', color: '#052e16', fontSize: '1.15rem', fontWeight: 800, textAlign: 'center',
        }}
      />
