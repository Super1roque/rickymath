...
export default function TerceroModulo03() {
  ...
  useEffect(() => {
    ...
    guardarProgresoModulo(user.uid, perfilActivo.id, 'tercero-modulo-03', {
  ...
  function comprobarDiana(p: PreguntaMult, boton: HTMLButtonElement) {
    const actual = dianas[p.numero]
    if (actual.evaluado || actual.valor.trim() === '') return
    const correcto = Number(actual.valor.trim()) === p.a * p.b
    if (correcto) { reproducirCorrecto(); disparar(String(p.a * p.b), boton.getBoundingClientRect()) } else reproducirIncorrecto()
  ...
  function comprobarCompleta(p: PreguntaCompleta, boton: HTMLButtonElement) {
    ...
    if (correcto) { reproducirCorrecto(); disparar(p.aceptables ? p.aceptables[0] : actual.valor.trim(), boton.getBoundingClientRect()) } else reproducirIncorrecto()
    registrarResultado(correcto)
    reaccionarRicky(correcto)
        ...
        <div style={{
          ...
          {DIANAS.map((p, i) => (
            <TarjetaDiana key={p.numero} p={p} estado={dianas[p.numero]} color={COLORES[i % COLORES.length]}
              ...
              onComprobar={boton => comprobarDiana(p, boton)} onExplicarEstado={reaccionarRickyExplicar} />
          ))}
        </div>
