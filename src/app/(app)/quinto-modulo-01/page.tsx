...
// C, D, U — hasta 9.999.999), pero el ítem 3 es un número de 8 cifras
// (12.500.000). Confirmado con el usuario: para ese ítem se agrega una
// columna DMi (decena de millón) al principio; los ítems 1, 2 y 4 quedan
// con las 7 columnas originales.
...
const ITEMS: ItemNumero[] = [
  { numero: 1, valor: '1.305.000', columnas: ['UMi', 'CM', 'DM', 'UM', 'C', 'D', 'U'], digitos: [1, 3, 0, 5, 0, 0, 0], escrito: 'un millon trescientos cinco mil' },
  { numero: 2, valor: '4.070.900', columnas: ['UMi', 'CM', 'DM', 'UM', 'C', 'D', 'U'], digitos: [4, 0, 7, 0, 9, 0, 0], escrito: 'cuatro millones setenta mil novecientos' },
  { numero: 3, valor: '12.500.000', columnas: ['DMi', 'UMi', 'CM', 'DM', 'UM', 'C', 'D', 'U'], digitos: [1, 2, 5, 0, 0, 0, 0, 0], escrito: 'doce millones quinientos mil' },
  { numero: 4, valor: '305.017', columnas: ['UMi', 'CM', 'DM', 'UM', 'C', 'D', 'U'], digitos: [0, 3, 0, 5, 0, 1, 7], escrito: 'trescientos cinco mil diecisiete' },
]
...
const COMPLETAR: PreguntaCompleta[] = [
  {
    numero: 1, antes: 'Mil millares forman un', despues: '.',
    aceptables: ['millon'],
    lectura: 'Mil millares, ¿qué forman?',
    explicacion: 'Mil millares (mil veces mil) forman un millón — el siguiente orden después de las centenas de mil.',
  },
  {
    numero: 2, antes: 'En 4.070.900, el 4 vale', despues: 'unidades.',
    aceptables: ['4000000'],
    lectura: 'En cuatro millones setenta mil novecientos, ¿cuántas unidades vale el 4?',
    explicacion: 'El 4 está en el lugar de las unidades de millón (UMi), así que vale 4 por 1.000.000, que es 4.000.000 unidades.',
    numerica: true,
  },
  {
    ...
    explicacion: 'Comparando los cuatro números, 12.500.000 es el más grande — tiene 8 cifras, más que los otros tres.',
    numerica: true,
  },
]
...
export default function QuintoModulo01() {
  ...
  function comprobarCompleta(p: PreguntaCompleta) {
    ...
    const correcto = p.numerica
      ? p.aceptables.includes(soloDigitos(actual.valor))
      : p.aceptables.map(normalizarTexto).includes(normalizarTexto(actual.valor))
    if (correcto) reproducirCorrecto(); else reproducirIncorrecto()
    registrarResultado(correcto)
    reaccionarRicky(correcto)
  ...
  return (
    <div className={fuenteJuego.className} style={{
      minHeight: '100vh', overflowX: 'hidden', background: 'linear-gradient(180deg, #ca8a04 0%, #fde68a 35%, #fef9c3 100%)',
      ...
      <div style={{
        position: 'relative',
        background: 'linear-gradient(135deg, #a16207, #713f12)', padding: '1.75rem 1rem',
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '1.5rem 1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 110 }}>
            <BarraProgreso completadas={totalEvaluadas} total={TOTAL_PREGUNTAS} color="#a16207" />
          </div>
        </div>
          ...
          <div style={{
            ...
            <p style={{ opacity: 0.75, marginBottom: '1.5rem', fontSize: '1rem', fontWeight: 600 }}>
              {totalCorrectas === TOTAL_PREGUNTAS
                ? '¡Perfecto! Dominás los números hasta el millón 🎮'
...
function TorresValorPosicional({ digitos, colores }: { digitos: number[]; colores: string[] }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'center', gap: '0.35rem', height: 80, marginBottom: '0.5rem' }}>
      {digitos.map((d, i) => (
        <div key={i} style={{
          width: 18, height: Math.max(8, d * 8), background: colores[i % colores.length],
...
// Ejemplo resuelto — el mismo ejemplo que trae la hoja original (2.406.180),
// así que no hace falta inventar uno nuevo ni hay riesgo de repetir un
// número de los 4 ítems reales.
function TarjetaEjemplo() {
  const color = '#3b82f6'
  const valor = '2.406.180'
  const columnas = ['UMi', 'CM', 'DM', 'UM', 'C', 'D', 'U']
  const digitos = [2, 4, 0, 6, 1, 8, 0]
  const escrito = 'dos millones cuatrocientos seis mil ciento ochenta'
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.7rem' }}>
        ...
        <BotonEscuchar texto={`Ejemplo: dos millones cuatrocientos seis mil ciento ochenta, separado en órdenes: unidad de millón 2, centena de mil 4, decena de mil 0, unidad de mil 6, centena 1, decena 8, unidad 0.`} tamano={32} />
      </div>

      <TorresValorPosicional digitos={digitos} colores={['#2dd4bf', '#a855f7', '#94a3b8', '#3b82f6', '#eab308', '#ec4899', '#94a3b8']} />

      <p style={{ textAlign: 'center', fontWeight: 800, fontSize: '1.4rem', color: '#1e3a8a', margin: '0 0 0.6rem' }}>{valor}</p>

      <div style={{ display: 'flex', gap: '0.25rem', justifyContent: 'center', marginBottom: '0.6rem', flexWrap: 'wrap' }}>
        {columnas.map((c, i) => (
          <div key={c} style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '0.65rem', fontWeight: 800, color: '#1e40af', marginBottom: '0.2rem' }}>{c}</div>
            <div style={{
              width: 32, height: 32, borderRadius: 8, border: '2px solid #93c5fd', background: '#eff6ff',
              display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '1rem', color: '#1e3a8a',
            }}>
              {digitos[i]}
            </div>
          </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.6rem', gap: '0.4rem' }}>
        ...
        <p style={{ fontSize: '1.15rem', fontWeight: 800, margin: 0, flex: 1, textAlign: 'center', color: '#713f12' }}>{p.valor}</p>
        <BotonEscuchar texto={`Leé, separá en órdenes y escribí con letras: ${p.valor}`} tamano={32} />
      </div>

      <div style={{ display: 'flex', gap: '0.25rem', justifyContent: 'center', marginBottom: '0.7rem', flexWrap: 'wrap' }}>
        {p.columnas.map((c, i) => (
          <div key={c} style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '0.62rem', fontWeight: 800, color: '#92400e', marginBottom: '0.2rem' }}>{c}</div>
            <input
              type="number"
              inputMode="numeric"
              ...
              placeholder="?"
              style={{
                width: 32, padding: '0.3rem', borderRadius: 8, border: `2px solid ${bordeColor}55`,
                background: '#fffbeb', color: '#713f12', fontSize: '1rem', fontWeight: 700, textAlign: 'center',
              }}
            />
          </div>
        ))}
      ...
      <input
        ...
        style={{
          ...
          background: '#fffbeb', color: '#713f12', fontSize: '0.95rem', fontWeight: 600,
        }}
      />
  ...
  return (
    <div style={{
      ...
      <input
        ...
        onKeyDown={handleKeyDown}
        style={{
          width: 140, padding: '0.4rem 0.6rem', borderRadius: 10, border: '2px solid #fcd34d',
