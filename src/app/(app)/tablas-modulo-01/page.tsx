...
// ── Datos de la actividad (basados en la hoja "Tabla del 1", serie bonus
// "Tablas de Multiplicar"). La hoja trae la tabla completa (1×1 a 1×10) ya
...
const TRUCO = 'El 1 es un espejo: el número no cambia nunca. ¡Multiplicar por 1 es dejarlo igual!'
...
const PRACTICA: ItemPractica[] = [
  { numero: 1, n: 7, resultado: BASE * 7 },
  { numero: 2, n: 3, resultado: BASE * 3 },
  { numero: 3, n: 9, resultado: BASE * 9 },
  { numero: 4, n: 5, resultado: BASE * 5 },
  { numero: 5, n: 10, resultado: BASE * 10 },
]
...
export default function TablasModulo01() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    guardarProgresoModulo(user.uid, perfilActivo.id, 'tablas-modulo-01', {
  ...
  return (
    <div className={fuenteJuego.className} style={{
      minHeight: '100vh', overflowX: 'hidden', background: 'linear-gradient(180deg, #db2777 0%, #f9a8d4 35%, #fce7f3 100%)',
      ...
      <div style={{
        ...
        textAlign: 'center', boxShadow: '0 5px 0 #500724', borderBottom: '4px solid #500724',
      }}>
        <BotonMenu href="/tablas-menu" />
        <div style={{ fontSize: '2.6rem', marginBottom: '0.3rem' }}>🦎1️⃣</div>
        <h1 style={{
          fontSize: 'clamp(1.4rem, 5.5vw, 2.4rem)', fontWeight: 800, letterSpacing: '0.01em',
          textShadow: '2px 2px 0 #500724', margin: 0, color: 'white',
        }}>
          ¡Tabla del 1!
        </h1>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        <h2 style={{ fontSize: '1.3rem', fontWeight: 800, margin: '1.5rem 0 1rem', textAlign: 'center', color: '#831843' }}>
          ✏️ Ahora vos
        </h2>
        ...
        {!terminado && (
          <div
            aria-hidden="true"
            style={{ position: 'fixed', bottom: 10, right: 10, zIndex: 40, pointerEvents: 'none', filter: 'drop-shadow(0 3px 0 rgba(80,7,36,0.15))' }}
          >
          ...
          <div style={{
            ...
            <p style={{ opacity: 0.75, marginBottom: '1.5rem', fontSize: '1rem', fontWeight: 600 }}>
              {totalCorrectas === TOTAL_PREGUNTAS
                ? '¡Perfecto! Dominás la tabla del 1 🎮'
...
// Fila de bloques contables — un cuadradito por unidad, hasta 10, para que
// se vea como en la hoja (bloques con el "1" adentro) sin depender de
// imágenes.
  ...
  return (
    <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
      {Array.from({ length: cantidad }, (_, i) => (
        <div key={i} style={{
          width: 20, height: 20, borderRadius: 4, background: COLOR, color: 'white',
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.6rem' }}>
        ...
        <p style={{ fontWeight: 800, fontSize: '1.05rem', margin: 0, flex: 1, color: '#831843' }}>La tabla del {BASE}</p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
        {TABLA.map(f => (
          <div key={f.n} style={{
            ...
            <span style={{ fontWeight: 800, fontSize: '0.95rem', color: '#831843', width: 74, flexShrink: 0 }}>
              {BASE} × {f.n} =
            </span>
            <div style={{ flex: 1, overflowX: 'auto' }}>
              <FilaBloques cantidad={f.resultado} />
            </div>
            <span style={{ fontWeight: 800, fontSize: '1.1rem', color: COLOR, width: 30, textAlign: 'right', flexShrink: 0 }}>
              {f.resultado}
            </span>
          </div>
        ))}
...
function CajaTruco() {
  return (
    <div style={{
      background: '#fdf2f8', border: `3px dashed ${COLOR}`, borderRadius: 18, padding: '1rem',
      marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.6rem',
    }}>
      <span style={{ fontSize: '1.6rem', flexShrink: 0 }}>💡</span>
      <div style={{ flex: 1 }}>
        <p style={{ margin: '0 0 0.2rem', fontWeight: 800, color: '#831843', fontSize: '0.95rem' }}>Truco</p>
        <p style={{ margin: 0, fontSize: '0.9rem', color: '#831843', fontWeight: 600 }}>{TRUCO}</p>
      </div>
...
function TarjetaPractica({ p, estado, onCambiar, onComprobar, onExplicarEstado }: {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '1.3rem', fontWeight: 800, margin: '0 0 0.5rem', color: '#831843' }}>
        {BASE} × {p.n} =
      </p>
      ...
      <input
        ...
        style={{
          ...
          background: '#fdf2f8', color: '#831843', fontSize: '1.15rem', fontWeight: 800, textAlign: 'center',
        }}
      />
